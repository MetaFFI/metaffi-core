...
std::unordered_map<std::string,std::shared_ptr<compiler_plugin_interface_wrapper>> compiler_plugins;
...
//--------------------------------------------------------------------
void compiler_plugin_interface_wrapper::init()
{
	(*this->pinit)();
}
//--------------------------------------------------------------------
void compiler_plugin_interface_wrapper::compile_to_guest(const char* idl_def_json, uint32_t idl_def_json_length,
...
//--------------------------------------------------------------------
void compiler_plugin_interface_wrapper::compile_from_host(const char* idl_def_json, uint32_t idl_def_json_length,
                                                          const char* output_path, uint32_t output_path_length,
...
//--------------------------------------------------------------------
std::shared_ptr<compiler_plugin_interface_wrapper> compiler_plugin_interface_wrapper::load(const std::string& lang)
{
	std::stringstream compiler_plugin_name;
	compiler_plugin_name << "metaffi.compiler." << lang;
	
	std::shared_ptr<compiler_plugin_interface_wrapper> loaded_plugin;
	auto compiler_plugin_iter = compiler_plugins.find(compiler_plugin_name.str());
	if(compiler_plugin_iter == compiler_plugins.end())
	{
		loaded_plugin = std::make_shared<compiler_plugin_interface_wrapper>(compiler_plugin_name.str());
		loaded_plugin->init();
