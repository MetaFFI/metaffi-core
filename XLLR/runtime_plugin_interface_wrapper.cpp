...
runtime_plugin_interface_wrapper::runtime_plugin_interface_wrapper(const std::string& plugin_filename_without_extension)
{
	...
	this->pload_callable = load_func<void**(void*, metaffi_types_with_alias_ptr, metaffi_types_with_alias_ptr, int8_t, int8_t, char**,uint32_t*)>(*plugin_dll, "load_callable");
...
//--------------------------------------------------------------------
void** runtime_plugin_interface_wrapper::load_callable(void* load_callable_context, metaffi_types_with_alias_ptr params_types, metaffi_types_with_alias_ptr retval_types, uint8_t params_count, uint8_t retval_count, char** err, uint32_t* err_len)
{
	*err = nullptr;
	*err_len = 0;
	return (*this->pload_callable)(load_callable_context, params_types, retval_types, params_count, retval_count, err, err_len);
}
//--------------------------------------------------------------------
