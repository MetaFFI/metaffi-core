...
private:
	...
	std::shared_ptr<boost::dll::detail::import_type<void*(const char*, uint32_t, const char*, uint32_t, int8_t, int8_t, char**, uint32_t*)>::type> pload_function;
...
public:
	 ...
	 */ 
	[[nodiscard]] void* load_function(const char* module_path, uint32_t module_path_len, const char* function_path, uint32_t function_path_len, int8_t params_count, int8_t retval_count, char** err, uint32_t* err_len) override;
